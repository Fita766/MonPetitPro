                        ...
                        <tr key={obs.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="p-4 text-right">
                            <div className="flex justify-end gap-2">
                              <button onClick={() => { setEditingObs(obs); setShowCustomResponsible(!uniqueResponsables.includes(obs.responsible_person)); }} className="text-slate-400 hover:text-primary transition" title="Modifier">
                                <Edit size={16} />
                              </button>
            ...
            <form onSubmit={handleEditSubmit} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div>
                  ...
                  {uniqueResponsables.length > 0 && !showCustomResponsible && (!editingObs.responsible_person || uniqueResponsables.includes(editingObs.responsible_person)) ? (
                    <select
                      required
                      value={editingObs.responsible_person}
                      onChange={(e) => {
                        if (e.target.value === 'NEW') {
                          setShowCustomResponsible(true);
                          setEditingObs({...editingObs, responsible_person: ''});
                        } else setEditingObs({...editingObs, responsible_person: e.target.value});
                      }}
                      className="w-full px-3 py-2 border border-slate-300 rounded focus:ring-2 focus:ring-primary outline-none"
                    >
                      <option value="" disabled>Sélectionner...</option>
                      {uniqueResponsables.map((r) => <option key={r as string} value={r as string}>{r as string}</option>)}
                    ...
                    <div className="relative">
                      <input
                        ...
                        value={editingObs.responsible_person}
                        onChange={(e) => setEditingObs({...editingObs, responsible_person: e.target.value})}
                        className="w-full px-3 py-2 border border-slate-300 rounded focus:ring-2 focus:ring-primary outline-none pr-16"
                      />
                      {uniqueResponsables.length > 0 && (
                        <button type="button" onClick={() => { setShowCustomResponsible(false); setEditingObs({...editingObs, responsible_person: ''}); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-primary hover:underline">
                          Retour liste
                        </button>
                      )}
                    </div>
