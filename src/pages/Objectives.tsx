...
export default function Objectives() {
  ...
  useEffect(() => {
    ...
    void supabase
      .from("operations")
      .select(
        "id, name, of_number, department, commune, address, total_housing_units, is_objective, objective_year, objective_housing_units, objective_management_date, contractual_delivery_date, management_expected_date, management_actual_date",
      )
      .order("name")
      .then(({ data, error: fetchError }) => {
        if (cancelled) return;
        if (fetchError) setError(fetchError.message);
        else setOperations((data as ObjectiveOperation[] | null) ?? []);
        setLoading(false);
      });
    return () => {
      cancelled = true;
  ...
  const availableYears = useMemo(() => {
    ...
    operations.forEach((operation) => {
      if (operation.objective_year) years.add(operation.objective_year);
      if (operation.management_actual_date)
        years.add(Number(operation.management_actual_date.slice(0, 4)));
    });
    return [...years].sort((left, right) => right - left);
  }, [operations]);

  const objectiveRows = useMemo(
    () => buildObjectiveRows(operations, year),
    [operations, year],
  );
  const rows = useMemo(
    () =>
      mode === "objectives"
        ? objectiveRows
        : mergeActualOutsideObjectives(objectiveRows, operations, year),
    [mode, objectiveRows, operations, year],
  );
  const objectiveTotal = objectiveRows.reduce(
    (sum, row) => sum + row.objectiveHousingUnits,
    0,
  );
  const actualTotal = rows.reduce(
    (sum, row) => sum + row.actualHousingUnits,
    0,
  );
  const gainLossTotal = objectiveRows.reduce(
    (sum, row) => sum + (row.gainLoss ?? 0),
    0,
  );
  const monthlyActual = Array.from({ length: 12 }, (_, month) =>
    rows
      .filter(
        (row) =>
          row.actualManagementDate &&
          Number(row.actualManagementDate.slice(5, 7)) - 1 <= month,
      )
      .reduce((sum, row) => sum + row.actualHousingUnits, 0),
  );
  ...
  const exportExcel = async () => {
    ...
    sheet.addRow([`OBJECTIFS DMO ${year}`]);
    ...
    rows.forEach((row) => {
      const source = operations.find((operation) => operation.id === row.id);
      const excelRow = sheet.addRow([
        (source as ObjectiveOperation & { of_number?: string | null })
          ?.of_number ?? "",
        row.department,
        row.commune,
        row.address,
        row.objectiveHousingUnits,
        row.contractualDeliveryDate,
        row.expectedManagementDate,
        row.actualManagementDate,
        ...row.months.map((month) => month.value),
        row.gainLoss,
      ]);
      row.months.forEach((month, index) => {
        if (month.realized)
          excelRow.getCell(9 + index).fill = {
            type: "pattern",
            pattern: "solid",
    ...
    sheet.addRow(["", "", "", "RÉEL CUMULÉ", "", "", "", "", ...monthlyActual]);
    ...
    anchor.href = url;
    anchor.download = `objectifs-dmo-${year}.xlsx`;
    anchor.click();
  ...
  const exportPdf = () => {
    ...
    document.text(`Objectifs DMO ${year}`, 14, 16);
    autoTable(document, {
      startY: 22,
      head: [
        [
    ...
    document.save(`objectifs-dmo-${year}.pdf`);
  };
  ...
  return (
    <div className="mx-auto max-w-[1800px] pb-12">
      <header className="mb-7 flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Objectif figé, mise en gestion réelle et logements-mois gagnés ou
            perdus.
          </p>
        </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
        <table className="min-w-[1900px] w-full text-left text-[11px]">
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={22} className="p-12 text-center text-slate-400">
                  Aucune opération objectif pour {year}.
                </td>
              </tr>
            )}
          </tbody>
          <tfoot>
            <tr className="bg-slate-100 font-black">
              ...
              {monthlyActual.map((total, month) => (
                <td
                  key={month}
                  className="px-2 py-3 text-center text-emerald-800"
                >
                  {total}
                </td>
              ))}
