...
export default function OperationDetail() {
    ...
    void Promise.all([
      ...
      ([
        ...
        if (firstError) setError(firstError.message);
          ...
          setObservations(
            ((observationResult.data ?? []) as DetailObservation[]).map(
              normalizeObservation,
            ),
          );
          setTypologies(
            (typologyResult.data as OperationTypology[] | null) ?? [],
          );
          setSubsidies((subsidyResult.data as OperationSubsidy[] | null) ?? []);
          setConditions(
            (conditionResult.data as SuspensiveCondition[] | null) ?? [],
          );
          setDocuments(
            (documentResult.data as OperationDocument[] | null) ?? [],
          );
          setReviewItems(
  ...
  const deleteOperation = async () => {
    if (
      ...
      return;
    const observationDelete = await supabase
      .from("observations")
      .delete()
      .eq("operation_id", operation.id);
    if (observationDelete.error) {
      setError(observationDelete.error.message);
      return;
    }
    const operationDelete = await supabase
      .from("operations")
      .delete()
      .eq("id", operation.id);
    if (operationDelete.error) setError(operationDelete.error.message);
    else navigate("/");
  };
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <Info label="CTX" value={operation.project_manager} />
            <Info label="COP" value={operation.operations_manager} />
            <Info label="Assistante" value={operation.assistant_name} />
            <Info label="Gestionnaire" value={operation.manager_name} />
            <Info label="Promoteur" value={operation.promoter_name} />
            <Info label="Gesprojet" value={operation.gesprojet_number} />
          </div>
        </section>
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-wider text-slate-800">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            <Info label="Total" value={operation.total_housing_units} />
            <Info
              label="Collectifs"
              value={operation.collective_housing_units}
            />
            <Info
              label="Individuels"
              value={operation.individual_housing_units}
            />
            <Info label="PLUS" value={operation.plus_units} />
            <Info label="PLAI" value={operation.plai_units} />
            <Info label="PLS" value={operation.pls_units} />
          </div>
          <p className="mt-4 text-xs text-slate-500">
            {operation.certification || "Certification non renseignée"} ·{" "}
            {operation.thermal_regulation || "Thermique non renseigné"} ·{" "}
            {typologies.length} ligne(s) typologie
          </p>
        </section>
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-wider text-slate-800">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <Info
              ...
              value={
                operation.initial_budget == null
                  ? null
                  : Number(operation.initial_budget).toLocaleString("fr-FR", {
                      style: "currency",
                      currency: "EUR",
                    })
              }
            ...
            <Info
              label="Atterrissage"
              value={
                operation.final_budget == null
                  ? null
                  : Number(operation.final_budget).toLocaleString("fr-FR", {
                      style: "currency",
                      currency: "EUR",
                    })
              }
          ...
          <div className="mt-4 space-y-2">
            {subsidies.slice(0, 3).map((subsidy) => (
              <div
                key={subsidy.id}
            ...
            {subsidies.length === 0 && (
              <p className="text-xs text-slate-400">Aucune subvention.</p>
            )}
          </div>
        </section>
      </div>
          ...
          {operation.is_objective && (
            <section className="rounded-2xl border border-teal-200 bg-teal-50 p-5">
              ...
              <p className="mt-2 text-sm text-teal-800">
                {operation.total_housing_units ?? 0} logements rattachés à
                l’objectif.
              </p>
            </section>
          )}
            ...
            <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
              ...
              <div className="mt-3 space-y-2">
                {conditions.map((condition) => (
                  <p key={condition.id} className="text-xs text-amber-900">
                    <strong>{condition.subject}</strong> ·{" "}
                    {condition.deadline_date
                      ? displayDate(condition.deadline_date)
                      : "sans butoir"}{" "}
                    {condition.completion_date ? "✓" : ""}
                  </p>
                ))}
              </div>
            </section>
