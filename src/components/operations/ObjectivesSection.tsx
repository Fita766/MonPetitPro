...
export default function ObjectivesSection({ form, onChange }: OperationSectionProps) {
  ...
  return (
    <section>
      ...
      <div className="max-w-2xl rounded-3xl border border-teal-200 bg-gradient-to-br from-teal-50 to-white p-7">
        <CheckField checked={form.is_objective} onChange={(value) => onChange('is_objective', value)}>Cette opération fait partie des objectifs DMO Travaux</CheckField>
        <div className="mt-5 max-w-xs"><FieldLabel>Année de l’objectif</FieldLabel><TextInput disabled={!form.is_objective} min={2000} max={2100} type="number" placeholder={String(currentYear)} value={form.objective_year} onChange={(event) => onChange('objective_year', event.target.value)} /></div>
