...
export default function SynthesisSection({ form, onChange, canEditField = () => true }: OperationSectionProps) {
  return (
    <section>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <div><FieldLabel>Enjeux et description du projet</FieldLabel><TextArea disabled={!canEditField('synthesis_description')} rows={12} value={form.synthesis_description} onChange={(event) => onChange('synthesis_description', event.target.value)} placeholder="Contexte foncier, nature de l’opération, certification, contraintes et enjeux…" /></div>
        <div><FieldLabel>Travaux supplémentaires significatifs</FieldLabel><TextArea disabled={!canEditField('significant_works')} rows={12} value={form.significant_works} onChange={(event) => onChange('significant_works', event.target.value)} placeholder="Une ligne par intervention, avec montant si nécessaire…" /></div>
      </div>
